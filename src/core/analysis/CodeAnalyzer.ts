...
export class AdvancedCodeAnalyzer {
    ...
    private async findFileDependencies(filePath: string): Promise<string[]> {
        try {
            ...
            if (fileType === 'typescript' || fileType === 'javascript' || fileType === 'react' || fileType === 'react-typescript') {
                // البحث عن عبارات الاستيراد
                const importRegex = /import\s+(?:[\w*{}\s,]+)\s+from\s+['"]([^'"]+)['"]/g;
                let match;
                while ((match = importRegex.exec(content)) !== null) {
                    const importPath = match[1];
                    if (!importPath.startsWith('.')) { continue; } // تجاهل المكتبات الخارجية
    ...
    private extractKeywords(task: string): string[] {
        ...
        const commonWords = ['a', 'an', 'the', 'in', 'on', 'at', 'to', 'for', 'with', 'by', 'about', 'like', 'through', 'over', 'before', 'between', 'after', 'since', 'without', 'under', 'within', 'along', 'following', 'across', 'behind', 'beyond', 'plus', 'except', 'but', 'up', 'out', 'around', 'down', 'off', 'above', 'near'];
        ...
        try {
            ...
            for (const file of allFiles) {
                try {
                    ...
                    if (stats.size > 1024 * 1024) { // تجاهل الملفات أكبر من 1 ميجابايت
                        continue;
                    }
                    ...
                    if (matchCount >= Math.ceil(keywordRegexes.length * 0.3)) { // على الأقل 30% من الكلمات المفتاحية
     ...
     * @returns اقتراحات التعديل
     */
    private async findEditLocationsInFile(filePath: string, fileAnalysis: FileAnalysis, keywords: string[]): Promise<EditSuggestion[]> {
        try {
            // قراءة محتوى الملف
            const content = await fs.readFile(filePath, 'utf-8');
